...
def assemble_dist(language, dist_dict, use_sgsdk, part_from, use_dylib):
    ...
    if use_sgsdk:
        ...
        if "Windows" in dist_dict['os']:
            if not os.path.exists(dist_source_folder+"bin/win32"):
                print >> sys.stderr, 'Missing Windows dll from', dist_source_folder+"bin/win32"
            else:
                copy_without_git(dist_source_folder+"bin/win32", specificdist_folder+"lib/win32", overwrite = False)
                copy_without_git(dist_source_folder+"bin/win64", specificdist_folder+"lib/win64", overwrite = False)
