...
{
    cout << "Testing Direct pixel drawing speed" << endl;
    _sg_functions->input.process_events();
...
void test_pixbuf(sg_drawing_surface *window_arr, int sz)
{
    cout << "Testing pixbuf drawing speed" << endl;
    _sg_functions->input.process_events();
    ...
    _sg_functions->input.process_events();
    sg_drawing_surface pixbuf = _sg_functions->pixbuf.create_pixbuf(800, 600);
    ...
    for (int cnt = 0; cnt < 5; cnt++)
    {
        for (int w = 0; w < sz; w++)
        {
            ...
            _sg_functions->graphics.clear_drawing_surface( &pixbuf, {1,1,1,1} );
            ...
            for (int y = 0; y < (120 * cnt); y++)
            {
                for (int x = 0; x < 800; x++)
                {
                    data[0] = x;
                    data[1] = y;
                    _sg_functions->graphics.draw_pixel(&pixbuf, {0,0,1,1}, data, 2 );
                }
            }
            ...
            _sg_functions->image.draw_bitmap(&pixbuf, &window_arr[w],srcDat,7,dstDat,4,SG_FLIP_NONE);
            ...
            cout << "    Pixel drawing time to pixbuf was " << mid_time - start_time << "ms" << endl;
            cout << "     Drawing time from pixbuf to window " << w  << " was "
                 << end_time - mid_time << "ms" << endl;
        }
    }
    _sg_functions->graphics.close_drawing_surface(&pixbuf);
    cout << endl;
}
